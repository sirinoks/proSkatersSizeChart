...
function handleChart() {
    ...
    var skate_brand = document.querySelector("#brand").selectedOptions[0].value;

    set_global_size_choise(cmSize, mmSize, mpSize, mondoSize, skate_brand, currentSize);
    update_table();
}

function set_global_size_choise(cm, mm, mp, mondo, skate_brand, currentSize) {
    console.log("set_global_size_choise");

    if (skate_brand != ""){
        my_size_skate_brand = skate_brand;
    }
    if (my_size_skate_brand == ""){
        my_size_skate_brand = brand_skates[0];
    }

    my_size_shoe_brand = "No Brand";
    brands_table.table_data["No Brand"] = [];

    brands_table.table_data["No Brand"].push({
        psp_sizing_cm: "0",
        psp_sizing_eu: "0",
        psp_sizing_mondo:"0",
        psp_sizing_mp:"0",
    ...
    if (currentSize.classList.contains("choiceCm")) {
        ...
        brands_table.table_data["No Brand"][0].psp_sizing_cm = parseFloat(cm) - 0.5;
        brands_table.table_data["No Brand"][1].psp_sizing_cm = cm;
        brands_table.table_data["No Brand"][2].psp_sizing_cm = parseFloat(cm) + 0.5;
    }
        ...
        my_size_mp = 0;
        my_size_mondo = 0;
        my_size_mm = parseFloat(my_size_mm)
        //if 160 probably user wanted CM
        if (my_size_mm < 160) {
            my_size_mm = mm*10;
        }
        brands_table.table_data["No Brand"][0].psp_sizing_cm = (my_size_mm - 5) / 10;
        brands_table.table_data["No Brand"][1].psp_sizing_cm = my_size_mm / 10;
        brands_table.table_data["No Brand"][2].psp_sizing_cm = (my_size_mm + 5) / 10;
    }
        ...
        my_size_mp = 0;
        my_size_mondo = mondo;
        my_size_mondo = parseFloat(my_size_mondo)
        //if 160 probably user wanted CM
        if (my_size_mondo < 160) {
            my_size_mondo = mondo*10;
        }
        brands_table.table_data["No Brand"][0].psp_sizing_cm = (my_size_mondo-5)/10;
        brands_table.table_data["No Brand"][1].psp_sizing_cm = my_size_mondo/10;
        brands_table.table_data["No Brand"][2].psp_sizing_cm = (my_size_mondo+5)/10;
    }
}
...
async function handleEventListeners() {
    ...
    document.querySelectorAll(".brandChoice").forEach(brand =>
        brand.addEventListener("click", (event) => {
            my_size_shoe_brand = event.target.innerText;
            return update_table();
        })
    )
}
...
async function update_global_data() {
    ...
    psp_calculator_plugin_add_brandChoice("No Brand");
}
